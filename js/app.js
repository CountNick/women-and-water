...
const init = async(config) => {
  ...
  const plotHomeLocation = (coordinates) => {
      ...
      storyElement.insertAdjacentHTML('afterbegin', `<button data-attribute="story__prev-btn" class="story__prev-btn">Previous</button> <button class="story__next-btn">Next</button> `)
      ...
      fetch(query)
          ...
          .then(data => {
              ...
              map.on('click', 'tilequery-points', (e) => console.log(randomWaterSource))
              // route source used: geoconverter.hsr.ch
              // fetch a route for the water spot
              fetch(`https://api.mapbox.com/directions/v5/mapbox/walking/${waterSource._data.geometry.coordinates[0]},${waterSource._data.geometry.coordinates[1]};${coordinates[0]},${coordinates[1]}?geometries=geojson&access_token=${mapboxgl.accessToken}`)
                .then(res => res.json())
                .then(data => {
...
function addLayers(map) {
  ...
  map.on('load', (e) => {
      ...
      map.addSource('mark', { // Add a new source to the map style: https://docs.mapbox.com/mapbox-gl-js/api/#map#addsource
          type: "geojson",
          data: {
      ...
      map.addSource('radius', { // Add a new source to the map style: https://docs.mapbox.com/mapbox-gl-js/api/#map#addsource
          type: "geojson",
          data: {
            "type": "FeatureCollection",
            "features": []
...
function updateStory(elements, event, config, map, setLayerOpacity) {
    ...
    const currentChapterElement = elements.find(child => child.classList.contains('active'))
    const currentChapterObject = config.chapters.find(chap => chap.id === currentChapterElement.id)
    ...
    console.log(nextChapter)
    const nextChapterElement = elements.find(child => child.id === nextChapter.id)
